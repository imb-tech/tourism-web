...
function CostItem({ day, data }: CostGeneratedItem) {
    ...
    return (
        <form>
            <CustomTable grid="grid-cols-9" className="py-">
                ...
                <div className="flex flex-col gap-1 col-span-8">
                    {fields?.map((col, i) => (
                        <div
                            className={cn(
                                "grid grid-cols-8 gap-2 py-2",
                                i > 0 ? "border-t border-secondary" : "",
                            )}
                            key={col.key}
                        >
                            <CustomTableCol>{data[i].full_name}</CustomTableCol>
                            <CustomTableCol>{data[i].phone}</CustomTableCol>
                            <CustomTableCol>
                            ...
                            <CustomTableCol>
                                <p className="flex gap-2">
                                    {data[i].languages?.map((l) => (
                                        <span
                                            className="bg-secondary px-2 py-1 rounded-md flex items-center w-auto"
                                            key={l}
                                        >
                                            {l}
                                        </span>
                            ...
                            <CustomTableCol>{data[i].desc}</CustomTableCol>
...
export function GuideList() {
    ...
    const { data, isLoading, isSuccess } = useGet<ICostItem[]>(
    ...
    return (
        <div className="bg-secondary p-3 flex flex-col gap-2 rounded-md">
            ...
            {isSuccess &&
                list?.map((item) => <CostItem key={item.id} {...item} />)}
        </div>
    )
}
