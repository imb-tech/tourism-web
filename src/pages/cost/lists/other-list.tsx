...
function CostItem({ day, data }: CostGeneratedItem) {
    ...
    return (
        <CustomTable grid="grid-cols-7" className="py-2">
            <div className="flex flex-col justify-center">
                <p className="text-primary">Day {day}</p>
            </div>
            <form className="flex flex-col col-span-6 gap-1">
                {fields?.map((col, i) => (
                    <div
                        className={cn(
                            "grid grid-cols-6 gap-2 py-2",
                            i > 0 ? "border-t border-secondary" : "",
                        )}
                        key={col.key}
                    >
                        <CustomTableCol>{data[i].category_name}</CustomTableCol>
