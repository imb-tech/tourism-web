...
function CostItem({ day, data }: CostGeneratedItem) {
    ...
    return (
        <CustomTable grid="grid-cols-10" className="py-2">
            <div className="flex flex-col justify-center">
                <p className="text-primary">Day {day}</p>
            </div>
            <form className="flex flex-col col-span-9 gap-1">
                {fields?.map((col, i) => (
                    <div
                        className={cn(
                            "grid grid-cols-9 gap-2 py-2",
                            i > 0 ? "border-t border-secondary" : "",
                        )}
                        key={col.key}
                    >
                        <CustomTableCol>
                            {data[i].transport_name}
                        </CustomTableCol>
                        <CustomTableCol>{data[i].size}</CustomTableCol>
                        <CustomTableCol>
                        ...
                        <CustomTableCol>{data[i].driver}</CustomTableCol>
                        <CustomTableCol>{data[i].driver_phone}</CustomTableCol>
                        <CustomTableCol>{data[i].company_phone}</CustomTableCol>
